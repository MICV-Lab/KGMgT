...
from kg_net.models.archs.arch_util import tensor_shift
from kg_net.models.archs.ref_map_util import feature_match_index
...
class FlowSimCorrespondenceGenerationArch(nn.Module):
    ...
    def forward(self, dense_features, img_ref_hr):
        ...
        for ind in range(img_ref_hr.size(0)):  # [9, 3, 160, 160]
            ...
            c, h, w = feat_in.size()
            feat_in = F.normalize(feat_in.reshape(c, -1), dim=0).view(c, h, w)  # view(c,h,w):
            ...
            similarity_relu3.append(sim_relu3)  # append()
