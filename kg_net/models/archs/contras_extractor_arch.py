...
class ContrasExtractorLayer(nn.Module):
    ...
    def __init__(self):
        ...
        features[0] = nn.Conv2d(2, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
        features[1] = nn.LeakyReLU(0.2, True)
        features[3] = nn.LeakyReLU(0.2, True)
        features[6] = nn.LeakyReLU(0.2, True)
        features[8] = nn.LeakyReLU(0.2, True)

        modified_net = OrderedDict()
        for k, v in zip(vgg16_layers, features):
            modified_net[k] = v
        # modified_net['conv1_1'].in_channels = 2
...
class ContrasExtractorSep(nn.Module):
    ...
    def __init__(self):
        ...
        self.feature_extraction_image2_1 = ContrasExtractorLayer()

    def forward(self, image1, image2_1):  #hwgm5
        dense_features1 = self.feature_extraction_image1(image1)
        dense_features2_1 = self.feature_extraction_image2_1(image2_1)
        return {
            'dense_features1': dense_features1,
            'dense_features2': dense_features2_1
        }
